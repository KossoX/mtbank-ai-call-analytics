...
def transcribe_audio(
    ...
    if not segments:
        raise RuntimeError(f"ASR returned no segments for: {audio_path}")
    ...
    if not transcript:
        raise RuntimeError(f"ASR returned an empty transcript for: {audio_path}")
...
def evaluate_dataset(
    ...
    if not calls_dir.is_dir():
        raise FileNotFoundError(f"Calls directory not found: {calls_dir}")
    ...
    if not audio_files:
        raise FileNotFoundError(f"No WAV files found in: {calls_dir}")
    ...
    for audio_path in audio_files:
        ...
        if not normalized_reference:
            raise ValueError(
                f"Reference became empty after normalization: {reference_path}"
            )

        if not normalized_hypothesis:
            raise RuntimeError(
                f"ASR hypothesis became empty after normalization: {audio_path}"
            )
...
def print_markdown_table(
    ...
    average_score = mean(score for _, score in results)

    print()
    print(f"Средний WER по набору: `{average_score:.4f}`")


def parse_args() -> argparse.Namespace:
    """Parse command-line arguments."""
    parser = argparse.ArgumentParser(
        description=("Calculate WER for test audio files using reference transcripts.")
    )
    ...
    parser.add_argument(
        ...
        help=("Directory containing WAV files. Default: test_data/calls"),
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Directory containing reference TXT files. Default: test_data/references"
        ),
    )
